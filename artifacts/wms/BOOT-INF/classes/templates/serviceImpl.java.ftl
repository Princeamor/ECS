// +----------------------------------------------------------------------
// | 壹号WMS企业级开发框架 [ 赋能开发者，助力企业发展 ]
// +----------------------------------------------------------------------
// | 版权所有 2020~2025 壹号WMS团队
// +----------------------------------------------------------------------
// | Licensed Apache-2.0 【壹号WMS】并不是自由软件，未经许可禁止去掉相关版权
// +----------------------------------------------------------------------
// | 官方网站: https://www.ehowms.com
// +----------------------------------------------------------------------
// | 软件作者: @壹号WMS团队 团队荣誉出品
// +----------------------------------------------------------------------
// | 版权和免责声明:
...
import com.ehowms.core.config.AppConfig;
import com.ehowms.core.utils.CommonUtils;
import com.ehowms.core.utils.R;
import com.ehowms.core.utils.StringUtils;
...
public class ${table.serviceImplName} extends ${superServiceImplClass}<${table.mapperName}, ${entity}> implements ${table.serviceName} {
    ...
    public Page<${table.entityName}> page(${table.entityName}PageDTO ${table.entityName?uncap_first}PageDTO) {
        ...
        LambdaQueryWrapper<${table.entityName}> wrapper = new LambdaQueryWrapper<${table.entityName}>()
			...
			<#list table.fields as field>
				<#if field.comment?contains('名称') || field.comment?contains('标题') || field.comment?contains('编码') || field.comment?contains('单号') || field.comment?contains('状态') || field.comment?contains('类型')>
    ...
    public R update(${table.entityName}UpdateDTO ${table.entityName?uncap_first}UpdateDTO) {
        ...
        if (StringUtils.isNull(${table.entityName?uncap_first})) {
            return R.failed("response.record_not_found");
        }
        // 属性拷贝
    ...
    public R batchDelete(List<Integer> idList) {
        ...
        if (StringUtils.isEmpty(idList)) {
            return R.failed("response.delete_record_id_not_found");
        }
        // 批量删除
